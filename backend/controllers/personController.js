...
let find = function (req, res, next) {
    personService.find(req.query).then((person) => {
        res.send(person);
    }).catch(next);
};

let update = function (req, res, next) {
    personService.update({ _id: req.params.id }, req.body)
        .then(person => res.send(person))
        .catch(next);
};

let remove = function (req, res, next) {
    personService.remove({ _id: req.params.id }).then((user) => {
        res.send(user);
    }).catch(next);
};

let create = function (req, res, next) {
    personService.create(req.body).then((user) => {
        res.send(user);
    }).catch(next);
};
...
module.exports = { find, update, remove, create, uploadImagem };